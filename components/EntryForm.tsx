  ...
  const fetchPastMeals = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/entries?patientId=${patientId}&entryType=meal&startDate=${thirtyDaysAgo.toISOString().split('T')[0]}&endDate=${new Date().toISOString().split('T')[0]}`,
      );
                  ...
                  <Box mt={1}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        startAdornment={
                          fetchingPreviousMeals ? <CircularProgress size={16} /> : <HistoryIcon />
                        }
                      >
                          ...
                          <MenuItem key={meal.id} value={meal.id}>
                            <Box>
                              ...
                              <Typography variant="caption" color="text.secondary">
                                ...
                                {meal.timeDiff > 0 &&
                                  ` (${meal.timeDiff} min ${meal.timeDiff === 1 ? 'difference' : 'difference'})`}
                              </Typography>
                            </Box>
                          </MenuItem>
                        ))}
